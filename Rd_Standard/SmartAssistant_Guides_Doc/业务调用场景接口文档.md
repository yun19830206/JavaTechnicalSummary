...
## 3-3：获得微信端登入账号信息
```javascript
    url: http://{host}/aiassistant/user/get/loginuser?wxCode=wxCode   
...
## 3-3：修改本人密码功能
```javascript
    url: http://{host}/aiassistant/user/change/password?newPassword=newPassword       
...
```javascript
    url: http://{host}/aiassistant/formdata/modify/formrowdata?wxcode=ldkclk5l5k    //微信提供的code
    
    parameter: JSON
        {
            "tableId" : 1,                //[必填]表单主键ID，由当面所在表单查询页面维护
            "tableName": "crm_customer"   //[必填]表单配置的表名称
            "dataId": 21                  //[必填]要修改表单数据ID
            "columnValueList": [          //[必填]属性名 属性值 数组,填入全部属性，扩展属性从表单配置属性中取。属性要全。create_user、create_time、tenant_id统一不传
                    {
                        "columnName":"compange_name",  
                        "columnValue":"南京云问"     
...
```javascript
    url: http://{host}/aiassistant/formdesign/list/authdesigntable?tableId=1&wxcode=ldkclk5l5k    //tableId=当前表单唯一标识
...
```
## 11-4：[已赋权数据--表单数据]获得表单数据 已经赋权给其他人员的信息
```javascript
...
## 11-5：[表单数据转交他人]将我表单的一条数据，转交给其他人
```javascript
    url: http://{host}/aiassistant/formdata/transfer/onedata
    
    parameter: 
        {
            "tableId": 1,                       //转交数据表单配置ID
            "tableName": "crm_customer",        //转交数据表的表名
            "dataId": 157,                      //转交表的数据ID
            "toUserId": 56                      //转交给谁
        }
         
    result: {
            "code":200,             //200转交成功，500失败
            "message":"转交成功",    //消息
            "debugMessage":null,
            "data":null
    }
...
```javascript
    url: http://{host}/aiassistant/business/crm/get/customer?customerid=1&wxcode=ldkclk5l5k    //customerid=crm客户ID
    ...
    result: 
    {
        ...
        "message":"请求成功",    //消息
        "debugMessage":null,
        "data":{                //Object，有3个属性 customer：客户信息， projectList：项目信息数组， contactPeopleList：联系人信息数组
            "customer":{   //customer：客户信息,每一个属性用只有一个属性的Object表示，以便于顺序正确
                {
                    "id":1
                },
                {
                    "compange_name":"广东美的制冷设备有限公司"
                },
                {
                    "其他属性":"其他值"
                }
            },
            "projectList":[     //projectList项目信息数组,每一个项目的每一个属性用只有一个属性的Object表示，以便于顺序正确
                [
                    {
                        "id":1
                    },
                    {
                        "compange_name":{
                            "displayValue":"广东美的制冷设备有限公司",
                            "originValue":1
                        }
                    },
                    {
                        "project_name":"美的IT智能客服项目"
                    }
                ]
            ],
            "contactPeopleList":[       //contactPeopleList联系人信息数组,每一个联系的每一个属性用只有一个属性的Object表示，以便于顺序正确
                [
                    {
                        "id":2
                    },
...
## 业务-2：CRM一个客户转交给其他人
```javascript
    url: http://{host}/aiassistant/business/crm/trans?customerId=156&toUserId=789
```

## 业务-3：推送消息：统一给每个人推送消息

## 业务-4：根据拜访记录信息，更新CRM客户的更新时间:http://{host}/aiassistant/business/crm/updatecustomer
## 业务-5：给一个访客推送消息:http://{host}/aiassistant/business/crm/pushmessage/wx/one?openId=123&message=hello word
## 业务-6：给所有销售推送更新客户信息:http://{host}/aiassistant/business/crm/pushmessage/wx/all
